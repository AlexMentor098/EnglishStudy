...
- (BOOL)excuteSQL:(NSString *)sql
{
    ...
    if ( rc == SQLITE_OK )
    {
        const char *utfsql = [sql UTF8String];
        char* err;
        if ( sqlite3_exec(mdb, utfsql, NULL, NULL, &err) == SQLITE_OK )
        {
            sqlite3_close(mdb);
            return YES;
        }
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orVAList:(va_list)args {
    ...
    if( !pStmt )
    {
        ...
        {
            retry   = NO;
            rc      = sqlite3_prepare_v2(mdb, [sql UTF8String], -1, &pStmt, 0);
            ...
            if (SQLITE_BUSY == rc) {
                ...
                if (outErr) {
                    *outErr = [NSError errorWithDomain:[NSString stringWithUTF8String:sqlite3_errmsg(mdb)] code:rc userInfo:nil];
                }
...
//    [self executeUpdate:insertSql withArgumentsInArray:arrArgs];
...
//    [self executeUpdate:deleteSql withArgumentsInArray:nil];
...
- (NSMutableArray *)queryArrayData:(NSString *)strQuery
{
    ...
    for( int nRow = 0; nRow < nRecordCount; nRow++ )
    {
        ...
        for( int nCol = 0; nCol < nColumnCount; nCol++ )
        {
            [dicRowItem setObject:[[arrColumns objectAtIndex:nCol] objectAtIndex:nRow] forKey:[arrayColume objectAtIndex:nCol]];
        }
